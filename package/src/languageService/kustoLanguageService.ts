...
if (typeof document == 'undefined') {
    ...
    importScripts('../../Portal/Content/Scripts/_oss/monaco-editor/0.18.1/min/vs/language/kusto/bridge.min.js');
    importScripts('../../Portal/Content/Scripts/_oss/monaco-editor/0.18.1/min/vs/language/kusto/kusto.javascript.client.min.js');
    importScripts('../../Portal/Content/Scripts/_oss/monaco-editor/0.18.1/min/vs/language/kusto/Kusto.Language.Bridge.min.js');
}
...
class KustoLanguageService implements LanguageService {
    ...
    setSchema(schema: s.Schema): Promise<void> {
        ...
        if (this._languageSettings.useIntellisenseV2) {
            let kustoJsSchemaV2: GlobalState =
                schema && (schema.clusterType === 'Engine' || schema.clusterType === 'ARG') ? this.convertToKustoJsSchemaV2(schema) : null;
    ...
    private static convertToKustoJsSchema(schema: s.Schema): k.KustoIntelliSenseQuerySchema | CmSchema | undefined {
        switch (schema.clusterType) {
            case 'ARG':
                const argCluster = new k.KustoIntelliSenseClusterEntity();
                const argDatabase = new k.KustoIntelliSenseDatabaseEntity();
                ...
                argDatabase.Tables = new Bridge.ArrayEnumerable(tables);

                argCluster.Databases = new Bridge.ArrayEnumerable([]);
                const argSchema = new k.KustoIntelliSenseQuerySchema(argCluster, argDatabase);
                return argSchema;
            case 'Engine':
    ...
    private convertToKustoJsSchemaV2(schema: s.EngineSchema | s.KustoARGSchema): GlobalState {
        if(schema.clusterType==="ARG") return GlobalState.Default;
    ...
    private createRulesProvider(
        ...
        if (clusterType == 'Engine'|| clusterType==='ARG') {
